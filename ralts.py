...
def data_viz():
	...
	if input_type == 'Text':
		...
		grouped_df_ent = grouped_df_ent.sort_values(by=('Relevance Score', 'mean'), ascending=False).reset_index()
		st.header('Entities')
		...
		grouped_df_topic = grouped_df_topic.sort_values(by=('Relevance Score', 'mean'), ascending=False).reset_index()
		st.header('Topics')
		...
		grouped_df_cat = grouped_df_cat.sort_values(by=('Relevance Score', 'mean'), ascending=False).reset_index()
